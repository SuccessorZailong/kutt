...
export interface EditBatchLink {
  ids: string[];
  target: string;
  batchEdit: Thunk<Links[], EditBatchLink>;
  updateIds: Action<EditBatchLink, string>;
  setTarget: Action<EditBatchLink, string>;
  setIds: Action<EditBatchLink, string[]>;
}
...
export const batchLink: EditBatchLink = {
  ids: [],
  target: null,
  batchEdit: thunk(async (actions, { ...payload }) => {
    const res = await axios.patch(
      `${APIv2.Links}/batchEdit`,
      payload,
      getAxiosConfig()
    );
    console.log(res);
