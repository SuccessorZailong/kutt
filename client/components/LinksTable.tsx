...
// 批量编辑弹窗
const BatchEditContent = styled(Col)`
  background-color: #e4f2fa;
  box-shadow: 0 5px 6px rgba(66, 165, 245, .5);
  position: fixed;
  top: 50%;
  left: 50%;
...
const checkFlex = { flexGrow: [1, 1, 1], flexShrink: [1, 1, 1] };
...
const Row: FC<RowProps> = ({ index, link, setDeleteModal }) => {
  ...
  const update = useStoreActions(s => s.links.update);
  const { updateIds } = useStoreActions(s => s.batchLink);
  ...
  link["checked"] = link["checked"] || false;
  const onChange = () => {
    const id = link.id;
    const checked = !link.checked;
    update({ id: id, checked: checked });
    updateIds(id);
    return checked;
  };
...
const LinksTable: FC = () => {
  ...
  const { get, remove, update } = useStoreActions(s => s.links);
  const { batchEdit, setIds } = useStoreActions(s => s.batchLink);
  const { ids } = useStoreState(s => s.batchLink);
  ...
  const [formState, { label, checkbox, text, raw }] = useFormState<Form>(
    {
      skip: "0",
      limit: "10",
      all: false,
    ...
    },
    {
      ...
      onChange(e, stateValues, nextStateValues) {
        if (stateValues.checks && !nextStateValues.checks) {
          formState.clear();
  ...
  const [batchEditMessage, setBatchEditMessage] = useMessage();
  const [remindMessage, setRemindMessage] = useMessage(2000);
  ...
  const onBatchEdit = async () => {
    if (batchEditLoading) return;
    setBatchEditLoading(true);
    try {
      // @ts-ignore
      await batchEdit({ ids: ids, target: formState.values.target });
      setShowBatchEdit(false);
      formState.clear();
      await get(options);
    } catch (err) {
      setBatchEditMessage(errorMessage(err));
    }
  ...
  const toggleBatchEdit = () => {
    if (ids.length > 0) {
      ...
      if (showBatchEdit) formState.reset();
    } else {
      setRemindMessage("Please select a link to bulk edit.");
      // setTimeout(() => {
      //   setRemindMessage("");
      // }, 2000);
    }
  ...
  return (
    <Col width={1200} maxWidth="95%" margin="40px 0 120px" mt={6} mb={5}>
      <H2 mb={3} light>
          ...
          <Th flexGrow={1} flexShrink={1}>
            ...
            <Flex ml={3} alignItems={"center"}>
              <Action
                ...
                strokeWidth="2.5"
                stroke="#2979ff"
                backgroundColor="#BFDDF4"
                onClick={toggleBatchEdit}
              />
              {remindMessage.text && (
                <Text fontSize={15} color={Colors.StopIcon}>
                  {remindMessage.text}
                </Text>
              )}
            </Flex>
              ...
              {...raw({
                ...
                onChange: () => {
                  ...
                  for (let item of links.items) {
                    const id = item["id"];
                    ids.push(id);
                    update({ id: id, checked: !formState.values.checks });
                  }
                  setIds(ids);
                  return !formState.values.checks;
                }
              })}
              checked={formState.values.checks}
              label=""
              ml={3}
              fontSize={[14, 15]}
              width={[15, 16]}
          ...
          <Col width={1} height={1}>
            <BatchEditContent>
              ...
              >
                ...
                >
                  <Icon
                    name={batchEditLoading ? "spinner" : "refresh"}
                    stroke="white"
                    mr={2}
                  />
                  {batchEditLoading ? "Updating..." : "Update"}
                </Button>
                {batchEditMessage.text && (
                  <Text mt={3} fontSize={15} color={batchEditMessage.color}>
                    {batchEditMessage.text}
                  </Text>
                )}
              </Col>
